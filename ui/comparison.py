...
def run_single_technique(name: str, query: str, top_k: int) -> dict[str, Any]:
    ...
    try:
        from src.task5_semantic_search import semantic_search_raw
        from src.task6_lexical_search import lexical_search
        from src.task7_reranking import rerank_rrf

        if name == "Dense Retrieval":
            docs = semantic_search_raw(query, top_k=top_k)
            ...
            dense = semantic_search_raw(query, top_k=top_k)
            ...
            dense = semantic_search_raw(query, top_k=top_k * 2)
...
def render_comparison_page(comp_top_k: int = 5) -> None:
    ...
    for idx, tech in enumerate(AVAILABLE_TECHNIQUES):
        with chip_cols[idx]:
            ...
            is_checked = st.checkbox(tech, key=state_key)
            if is_checked:
    ...
    auto_query = st.session_state.pop("comparison_pending_query", None)
    if auto_query is not None:
        st.session_state.comp_text_input = auto_query
    ...
    for s_idx, eq in enumerate(example_queries):
        with sugg_cols[s_idx]:
            if st.button(eq, key=f"btn_comp_sugg_{s_idx}", use_container_width=True):
                st.session_state.comparison_pending_query = eq
                st.rerun()

    # Trigger comparison run
    if (run_btn or auto_query is not None) and comp_query.strip():
