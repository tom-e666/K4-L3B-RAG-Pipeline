...
def format_citations_to_badges(answer_text: str) -> str:
    ...
    def repl1(match):
        ...
        return f'<span class="citation-badge">📌 Nguồn: {html.escape(friendly)}</span>'
...
def render_source_cards(sources: list[dict[str, Any]]) -> None:
    ...
    with st.expander(f"▸ Nguồn tham khảo ({len(sources)})", expanded=False):
        for index, source in enumerate(sources, 1):
            ...
            link_html = (
                f'<a href="{html.escape(str(url), quote=True)}" target="_blank" rel="noopener noreferrer" style="color: #2563EB; font-weight:600; text-decoration: none;">Mở liên kết ↗</a>'
