...
def render_sidebar() -> tuple[str, dict]:
    ...
    with st.sidebar:
        ...
        if "Chat" in selected_page:
            ...
            st.markdown(
                ...
                <div style="font-size: 0.8rem; color: #64748B; line-height: 1.45;">
                    So sánh đối sánh <b>Config A (Dense-only)</b> và <b>Config B (Hybrid + RRF)</b> trên 16 câu hỏi Golden Dataset.
                </div>
                """,
                unsafe_allow_html=True,
            )
        ...
        st.markdown(
            """
            <div style="position: absolute; bottom: 15px; left: 18px; right: 18px; font-size: 0.72rem; color: #94A3B8; text-align: center; border-top: 1px solid #F1F5F9; padding-top: 10px;">
